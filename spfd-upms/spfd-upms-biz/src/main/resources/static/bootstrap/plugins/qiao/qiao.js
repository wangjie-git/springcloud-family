...
qiao.bs.msgstr = function(msg, type, position){
	return '<div class="alert alert-'+type+' alert-dismissible" role="alert" style="display:none;position:fixed;' + position + ':0;left:0;width:100%;z-index:2001;margin:0;text-align:center;" id="bsalert" name="bsalert"><button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>'+msg+'</div>';
};
qiao.bs.msg = function(options){
	...
	$('div[name=bsalert]').slideDown();
	setTimeout(function(){
		$('div[name=bsalert]').slideUp(function(){
			$('div[name=bsalert]').remove();
		});
	},opt.time);
};
qiao.bs.popoptions = {
