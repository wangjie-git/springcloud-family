     ...
     table.render();
      ...
      //展示已知数据
    	 table.render({
    	        ...
    	        ,url : "../../person/queryInfos"
    	        ,cols: [[ //标题栏
    	          {field:'name', title: '姓名'},
    	          {field:'sex', title: '性别'},
    	          {field:'idNumber', title: '身份证号',width:185},
    	          {field:'telePhone', title: '电话号码'},
    	          {field:'orgName', title: '机构'},
    	          {field:'doctorName', title: '建档医生'},
    	          {field:'createDate', title: '建档时间'},
    	          {field:'status', title: '签约状态'},
...
/*    function removeTabelCell(){
    ...
    table.on('tool(person-table-operate)', function(obj){
        ...
        if(obj.event === 'fwjl'){
            location.href="serviceList.html?personId="+data.id;
        }
    });
