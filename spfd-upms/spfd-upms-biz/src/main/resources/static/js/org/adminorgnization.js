...
function initOrgTree() {
    $(".orgTree").combotree({
        url : "../orgnization/regionList?id=" + currentParentOrgId,
        valueField : 'id',
        textField : 'text',
        editable : false,
...
function addOrg() {
    ...
    $(".orgTree").combotree('setValue', "ks001");
}
    ...
    $("#orgParentSelect").combotree({
     /*   url : "../orgnization/regionList",
        ...
        onLoadSuccess : function(node, data) {
            $('#citySel').combotree('tree').tree("collapseAll");
        },
        onBeforeExpand : function(node) {
            $(".orgTree").combotree("tree").tree("options").url = "../orgnization/regionList";
        }
*/
        ...
        onBeforeExpand : function(node) {
            $("#orgParentSelect").combotree("tree").tree("options").url = "../orgnization/regionList";
        },
        onLoadSuccess: function(node, data) {
            //$('#orgParentSelect').combotree('setValue', currentOrgId);
            $('#orgParentSelect').combotree('setValue', "");
            $("#parentId").val(currentOrgId);
        }
   });
}
            ...
            if(allLabel!=null&&allLabel.length>0){
            	...
            	for(var i = 0 ;i<allLabel.length;i++){
            		...
            		RQRStd = "<td align=right>"+label.labelName+"</td>"+
                     ...
                     "<input style='width: 120px;float: left;' type='number' maxlength='6' class='input-sm form-control'"+
            ...
            if(allPack!=null&&allPack.length>0){
            	...
            	for(var i = 0 ;i<allPack.length;i++){
            		...
            		FWBSXtd = "<td align=right>"+pack.packName+"</td>"+
                     ...
                     "<label><input  type='radio' value='N' id='is"+pack.packId+"n' name='check"+pack.packId+"' "+isCheckedN+" />否</label>"+
                     "</div>"+
                     "<input type='hidden' id='FWBSX"+pack.packId+"id' name='FWBSX"+pack.packId+"id'/>"+
                     "<input style='width: 110px;float: right;' type='number' maxlength='6' class='input-sm form-control'"+
            		...
            		if((i+1)%2==0){
            			...
            			$("#targetTable").find("tr").eq(index+4).after(FWBRStr);
            			FWBRStr = new String("");
            			FWBSXtr = new String("");
