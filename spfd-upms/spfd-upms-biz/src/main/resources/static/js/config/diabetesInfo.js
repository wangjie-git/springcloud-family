...
function save(){
    ...
    $.ajax({
        ...
        data : param, // 将Json对象序列化成Json字符串，toJSON()需要引用jquery.json.min.js
        success : function(data) {
            window.location = xpath + "config/jumpConfigsDiabetes";
            alert("保存健康档案配置成功!");
        },
        error : function(res) {
            alert("保存健康档案配置失败!");
...
function doconfirm(){
    window.location = xpath + "config/loadDefaultConfigsDiabetes";
}
...
$("#menuType").change(function () {
    openclose ($("#menuType").attr("data-text"));
});

function openclose (t1){
    $("#menuType").find("option").each(function(){
            if(t1 != $(this).attr("data-text")){
