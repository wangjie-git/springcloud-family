layui
		.use(
				[ 'form', 'layedit', 'laydate', 'laytpl', 'table' ],
				function() {
					var form = layui.form, layer = layui.layer, layedit = layui.layedit, laydate = layui.laydate, laytpl = layui.laytpl, table = layui.table, $ = layui.$;
					...
					form
							.on(
									...
									$
												.ajax({
													url : "../../personIndex/saveOrUpdateGenes",
													type : "POST",
													data : data.field,
													dataType : "json",
													success : function(result) {
														...
														// 设置延迟1.2秒钟加载
														setTimeout(
																"window.location.href='../gene/index.html'",
																1200);
													},
													error : function(msg) {
														layer.msg('未连到服务器')
