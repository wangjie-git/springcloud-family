...
(function($) {
	...
	Jconfirm.prototype = {
		...
		animations : [ "anim-scale", "anim-top", "anim-bottom", "anim-left",
				"anim-right", "anim-zoom", "anim-opacity", "anim-none",
				"anim-rotate", "anim-rotatex", "anim-rotatey", "anim-scalex",
				"anim-scaley" ],
		_buildHTML : function() {
			var that = this;
			...
			this.$el = $(this.template).appendTo(this.container).addClass(
					this.theme);
			this.$el.find(".jconfirm-box-container").addClass(this.columnClass);
			this.CSS = {
				"-webkit-transition-duration" : this.animationSpeed / 1000
						+ "s",
				"transition-duration" : this.animationSpeed / 1000 + "s",
				"-webkjit-transition-timing-function" : "cubic-bezier(0.27, 1.12, 0.32, "
						+ this.animationBounce + ")",
				"transition-timing-function" : "cubic-bezier(0.27, 1.12, 0.32, "
						+ this.animationBounce + ")"
			};
			this.$el.find(".jconfirm-bg").css(this.CSS);
			this.$b = this.$el.find(".jconfirm-box").css(this.CSS).addClass(
					this.animation);
			if (this.rtl) {
			...
			if (this.confirmButton && this.confirmButton.trim() !== "") {
				this.$confirmButton = $(
						'<button class="btn">' + this.confirmButton
								+ "</button>").appendTo(this.$btnc).addClass(
						this.confirmButtonClass);
			}
			if (this.cancelButton && this.cancelButton.trim() !== "") {
				this.$cancelButton = $(
						'<button class="btn">' + this.cancelButton
								+ "</button>").appendTo(this.$btnc).addClass(
		...
		setContent : function(string) {
			...
			if (typeof this.content === "boolean") {
				if (!this.content) {
					...
					console
							.error("Invalid option for property content: passed TRUE");
				}
			} else {
				if (typeof this.content === "string") {
					if (this.content.substr(0, 4).toLowerCase() === "url:") {
						...
						var url = this.content
								.substring(4, this.content.length);
					...
					if (typeof this.content === "function") {
						...
						if (typeof promise !== "object") {
							console
									.error("The content function must return jquery promise.");
						} else {
							if (typeof promise.always !== "function") {
								console
										.error("The object returned is not a jquery promise.");
							} else {
								promise.always(function(data, status) {
									that.$btnc.find("button").removeAttr(
											"disabled");
									that.setDialogCenter();
								});
							}
						...
						console
								.error("Invalid option for property content, passed: "
										+ typeof this.content);
					}
				}
			}
		...
		_startCountDown : function() {
			...
			if (/cancel/.test(opt[0]) && this.type === "alert") {
				...
				if (/confirm|cancel/.test(opt[0])) {
					this.$cd = $('<span class="countdown">').appendTo(
							this["$" + opt[0] + "Button"]);
					var that = this;
					...
					console.error("Invalid option " + opt[0]
							+ ", must be confirm/cancel");
				}
			}
		},
		...
		setDialogCenter : function(animate) {
			...
			if (boxHeight > (windowHeight - minMargin)) {
				...
				var style = {
					"margin-top" : topMargin-100 //改变弹出框居中位置离顶部的距离
				};
			}
			if (animate) {
